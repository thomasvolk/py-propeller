...
2. Note-value validation: iterate `self.notes` with a 1-based counter and check `velocity ∈ [0, 127]` on each `Note` instance (velocity only — Q-1 resolved); raise `PropellerValidationError` with the 1-based position on the first failure.
